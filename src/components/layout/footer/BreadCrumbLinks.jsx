/**
 * @file BreadCrumbLinks.jsx
 * @module UI/BreadCrumbLinks
 * @desc Reusable breadcrumb navigation component for secondary page navigation.
 *       Displays a list of links with optional icons and separator slashes.
 *
 * @props {string} title - Title displayed above the breadcrumb links.
 * @props {Array} list - Array of breadcrumb link items.
 * @props {string} translationNamespace - Translation namespace used for link labels.
 *
 * @author Chace Nielson
 * @created Apr 1, 2025
 * @updated July 7, 2026 - set up translations
 */

import { useTranslations } from "next-intl";

import LinkItem from "../navbar/LinkItem";

export default function BreadCrumbLinks({ title, list, translationNamespace }) {
  const t = useTranslations(translationNamespace);

  return (
    <div>
      <h3 className="text-xl font-semibold mb-2">{title}</h3>

      <ul className="flex flex-wrap items-center gap-2 text-sm text-white">
        {list.map(({ labelKey, label, icon, href, router, scrollTo }, idx) => (
          <li key={idx} className="flex items-center">
            <LinkItem
              scrollTo={scrollTo}
              href={href}
              router={router}
              className="flex items-center gap-1 hover:text-accent transition-colors duration-100"
            >
              {icon && <span className="text-base">{icon}</span>}
              {labelKey ? t(labelKey) : label}
            </LinkItem>

            {idx < list.length - 1 && (
              <span className="px-1 pl-2.5 text-gray-300">/</span>
            )}
          </li>
        ))}
      </ul>
    </div>
  );
}